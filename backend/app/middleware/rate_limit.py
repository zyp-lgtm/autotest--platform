...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    RATE_LIMITS = {
        ...
        "/api/v1/keywords": (100, 60),  # 100 次/分钟 - 关键字查询

        # 项目和测试数据：宽松限制（频繁访问）
        "/api/v1/projects": (200, 60),  # 200 次/分钟 - 项目列表
        "/api/v1/test-data": (200, 60),  # 200 次/分钟 - 测试数据管理

        # 执行端点：中等限制
        "/api/v1/tasks": (50, 60),  # 50 次/分钟 - 任务执行
        "/api/v1/executions": (50, 60),  # 50 次/分钟 - 执行记录

        # 默认限制
        "default": (200, 60)  # 200 次/分钟
    }
